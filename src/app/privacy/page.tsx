...
export default function PrivacyConsentPage() {
  ...
  const canSubmit = useMemo(
    () => year.trim() && grade.trim() && classNum.trim() && number.trim() && name.trim() && agreed && !submitting,
    [year, grade, classNum, number, name, agreed, submitting]
  );
  ...
  async function submitConsent() {
    ...
    try {
      const response = await fetch("/api/registrations", {
        ...
        body: JSON.stringify({ year, grade, classNum, number, name }),
      });
